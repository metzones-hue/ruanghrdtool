...
export default function UMPage() {
  ...
  const getUMData = (kId: number): UMResult => {
    const pg = store.pengaturan || defaultPengaturan;
    const k = karyawan.find(x => x.id === kId);
    if (!k) return { hadir: 0, alpha: 0, umRate: 40000, umKotor: 0, potonganKB: 0, potonganDD: 0, potonganTelat: 0, umBersih: 0, lemburPeriode: 0, lemburCount: 0, detailRows: [] };
    ...
    if (umMode === 'minggu' && periodeVal) {
      ...
      const totalMenitTelat = abs.reduce((s, a) => s + (a.menitTelat || 0), 0);
      const potonganTelat = k.divisi !== 'HO' ? totalMenitTelat * pg.potonganTelat : 0;
      const umBersih = Math.max(0, umKotor - potonganKB - potonganDD - potonganTelat);
      ...
      return { hadir, alpha, umRate, umKotor, potonganKB, potonganDD, potonganTelat, umBersih, lemburPeriode, lemburCount, detailRows };
    } else {
  ...
  const totalPotonganTelat = aktif.reduce((s, k) => s + getUMData(k.id).potonganTelat, 0);
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button size="sm" onClick={() => { aktif.forEach(k => { if (!isBayar(k.id)) tandaiUMBayar(k.id, periodeVal); }); toast.success('Semua uang makan ditandai dibayar'); }} className="bg-emerald-500 hover:bg-emerald-600 text-white">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        {totalPotonganTelat > 0 && <Card className="bg-white dark:bg-neutral-900 border-gray-200 dark:border-neutral-800"><CardContent className="p-3"><p className="text-gray-400 dark:text-neutral-600 text-xs">Pot. Telat</p><p className="text-red-500 text-xl font-bold">-{fRp(totalPotonganTelat)}</p></CardContent></Card>}
          ...
          <table className="w-full">
            ...
            <tbody>
              {aktif.map((k, i) => {
                ...
                return (
                  <tr key={k.id} className="border-b border-gray-200 dark:border-neutral-800 hover:bg-gray-50 dark:hover:bg-neutral-900/50">
                    ...
                    <td className="py-2 px-2 text-right text-red-500 text-sm">{data.potonganTelat > 0 ? `-${fRp(data.potonganTelat)}` : '-'}</td>
              ...
              <tr className="bg-amber-500/5">
                ...
                <td className="py-3 px-2 text-right text-red-500 text-sm font-bold">{totalPotonganTelat > 0 ? `-${fRp(totalPotonganTelat)}` : '-'}</td>
        ...
        <div ref={slipRef}>
          {karyawanPrintData && printData && (
            <SlipUM
              ...
              potonganTelat={printData.potonganTelat}
